...
(function(Ps) {
    ...
    var P = {
        ...
        //组合效果映射器
        reflectEasy: function(effect, level) {
            var fun = this.lib.config.getEasyFun(effect).actName;
            return this.definedPs[effect] || this.lib.easy.getFun(fun, level);
        },
    ...
    window[Ps].prototype = {
        ...
        //组合效果
        ps: function(effect, level) {
            if (effect == "原图" || effect == "origin" || effect == "") {
                return this;
            }

            var fun = P.reflectEasy(effect, level);
...
(function(Ps) {
    ...
    window[Ps].module("easy", function(P) {
        ...
        var M = {
            getFun: function(fun, level) {
                level = level || 80;
                var Effects = {
                    softenFace: function() { //美肤
                        var _this = this.clone();
                        return _this.add(
                            this.act("高斯模糊", 15 * level / 100), "滤色", level > 100 ? "100%" : level + "%"
                        ).act("亮度", -10 + 10 * level / 100, 0 * level / 100);
                    },
                    sketch: function() { //素描
                        var _this = this.clone();
                        return this.add(
